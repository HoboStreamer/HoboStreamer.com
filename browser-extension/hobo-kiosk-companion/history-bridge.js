/**
 * Content script injected into hobostreamer.com/kiosk. It is the bridge between
...
function announce() {
  window.postMessage({ source: "hobo-kiosk-ext", type: "ready" }, ORIGIN);
}
...
window.addEventListener("message", (event) => {
  ...
  const d = event.data;
  if (!d || d.source !== "hobo-kiosk") return;

  if (d.type === "ping") { announce(); return; }

  if (d.type === "history-query") {
    const { q, id } = d;
    api.runtime.sendMessage({ type: "hobo-history-query", q })
      .then((resp) => {
        window.postMessage({ source: "hobo-kiosk-ext", type: "history-result", id, q, items: (resp && resp.items) || [] }, ORIGIN);
      })
      .catch(() => {
        window.postMessage({ source: "hobo-kiosk-ext", type: "history-result", id, q, items: [] }, ORIGIN);
      });
  }
});
