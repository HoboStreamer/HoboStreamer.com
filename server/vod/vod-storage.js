/**
 * HoboStreamer — VOD Storage Engine (local / Backblaze B2 / Cloudflare R2)
 *
 * Replaces the old filesystem-only hot/cold tier manager with native object
 * storage (inspired by openvibe-storage's provider/policy design):
 *
...
const PROVIDER_ENV = {
    b2: {
        endpoint: process.env.VOD_B2_ENDPOINT || '',
        region: process.env.VOD_B2_REGION || 'us-west-004',
        bucket: process.env.VOD_B2_BUCKET || '',
        accessKeyId: process.env.VOD_B2_KEY_ID || '',
        secretAccessKey: process.env.VOD_B2_APP_KEY || '',
        forcePathStyle: true,
    },
    r2: {
        endpoint: process.env.VOD_R2_ENDPOINT || '',
        region: process.env.VOD_R2_REGION || 'auto',
        bucket: process.env.VOD_R2_BUCKET || '',
        accessKeyId: process.env.VOD_R2_ACCESS_KEY_ID || '',
        secretAccessKey: process.env.VOD_R2_SECRET_ACCESS_KEY || '',
...
// ── Settings (site_settings, same keys the admin UI already edits) ──
const DEFAULTS = {
    enabled: true,
...
function setSetting(key, value) {
    ...
    if (existing) db.run('UPDATE site_settings SET value = ? WHERE key = ?', [JSON.stringify(value), dbKey]);
    else db.run('INSERT INTO site_settings (key, value) VALUES (?, ?)', [dbKey, JSON.stringify(value)]);
}
...
/** Offload a local VOD to B2 (cold). Alias of the old moveToCold. */
...
/** Restore an offloaded VOD to local disk. Alias of the old moveToHot. */
...
/** Delete a VOD's media everywhere (local + B2 + R2). */
...
async function runSweep() {
    ...
    try {
        ...
        const summary = {
            ...
            migrated,
            moved: migrated,                    // admin UI field
            promoted,
            demoted,
            bytesFreed,
            freedBytes: bytesFreed,             // admin UI field
            underPressure,
...
/**
 * One-time migration from the legacy hot/cold columns:
 *   - adds storage_provider/storage_key columns (done in database.js migration)
 *   - rows with storage_tier='cold' were uploaded to B2 at vods/<basename>
 *     by the old rclone mount — verify and flip them to provider 'b2'
 */
...
// ── Actual bucket usage + cost estimate (admin panel) ─────────

// Storage list prices ($/GB/month) and egress notes.
const CLOUD_PRICING = {
...
/**
 * Scan each configured object-store bucket for real usage (object count + bytes, broken
 * down by top-level prefix). Cached for 10 min — a full ListObjectsV2 walk is expensive.
 */
...
/** Estimate monthly cost from bucket usage using list prices. */
...
function getStatus() {
    ...
    return {
        ...
        // hot/cold keys kept for the existing admin UI
        hot: {
            disk: localDisk,
            vods: { bytes: localStats.bytes, files: localStats.files },
            dbCount: counts.localCount || 0,
            dbBytes: counts.localBytes || 0,
        },
        cold: {
            disk: { total: 0, used: Number(counts.b2Bytes || 0) + Number(counts.r2Bytes || 0), available: 0, usePct: 0, mount: 'b2+r2' },
            vods: { bytes: Number(counts.b2Bytes || 0) + Number(counts.r2Bytes || 0), files: (counts.b2Count || 0) + (counts.r2Count || 0) },
            dbCount: (counts.b2Count || 0) + (counts.r2Count || 0),
            dbBytes: Number(counts.b2Bytes || 0) + Number(counts.r2Bytes || 0),
            available: providerHealthy.b2 !== false && providerConfigured('b2'),
        },
        tiers: {
...
/** Which tier label to show for a VOD row in the admin listing. */
function tierLabel(vod) {
    const p = providerOf(vod);
    return p === 'local' ? 'hot' : p === 'b2' ? 'cold' : 'r2';
}
...
// ── CLI: node server/vod/vod-storage.js drain [targetPct] ────
