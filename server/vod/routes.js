...
// ── Create Clip ──────────────────────────────────────────────
function applyDefaultClipVisibility(clipId, ownerUserId) {
    if (!clipId || !ownerUserId) return;
    const ownerChannel = db.getChannelByUserId(ownerUserId);
    if (ownerChannel && ownerChannel.default_clip_visibility === 'public') {
...
router.post('/clips', requireAuth, vodUpload.single('video'), async (req, res) => {
    try {
        ...
        const parsedStartTime = req.body.start_time === undefined ? undefined : Number(req.body.start_time);
        const parsedEndTime = req.body.end_time === undefined ? undefined : Number(req.body.end_time);
        ...
        if (req.file) {
            ...
            const safeStartTime = Number.isFinite(parsedStartTime) ? parsedStartTime : 0;
            const safeEndTime = Number.isFinite(parsedEndTime) ? parsedEndTime : 0;
            ...
            const clipStream = parsedStreamId ? db.getStreamById(parsedStreamId) : null;
            applyDefaultClipVisibility(clipId, clipStream?.user_id || req.user.id);
        ...
        if (parsedVodId) {
            ...
            const isOwnerOrAdmin = vod.user_id === req.user.id || req.user.role === 'admin';
            if (!vod.is_public && !isOwnerOrAdmin) {
                return res.status(403).json({ error: 'This VOD is not available for clipping' });
            }
            if (vod.is_recording) {
                return res.status(400).json({ error: 'Live VODs must be clipped from the live player' });
            }

            const clipsDir = path.resolve(config.vod.clipsPath);
            if (!fs.existsSync(clipsDir)) fs.mkdirSync(clipsDir, { recursive: true });

            const clipExt = path.extname(vod.file_path) || '.webm';
            const clipFilename = `clip-${Date.now()}${clipExt}`;
            ...
            ffmpeg.on('exit', (code) => {
                ...
                const clipId = result.lastInsertRowid;
                applyDefaultClipVisibility(clipId, vod.user_id);
