...
function _pageMeta(routePath) {
    ...
    // List pages
    if (p === '/vods') return _listMeta('vods', 'VODs', 'Browse recorded live streams (VODs) on HoboStreamer — auto-recorded broadcasts with AI overviews and searchable transcripts.', () => (db.getRecentVods(30, 0) || []).map(v => ({ url: `/vod/${v.id}`, name: v.title || 'VOD', by: v.display_name || v.username, meta: _fmtDur(v.duration_seconds), desc: v.ai_overview_short })));
    if (p === '/clips') return _listMeta('clips', 'Clips', 'Watch the best clips from HoboStreamer live streams — highlights clipped by viewers and auto-generated by AI.', () => (db.getPublicClips(30, 0) || []).map(c => ({ url: `/clip/${c.id}`, name: c.title || 'Clip', by: c.display_name || c.username || c.streamer_username, meta: _fmtDur(c.duration_seconds), desc: c.ai_overview_short })));
    if (p === '/pastes') return _listMeta('pastes', 'Pastes', 'Code, text and screenshot pastes shared on HoboStreamer — a Pastebin built into the streaming network, with AI summaries.', () => { const r = db.listPastes({ visibility: 'public', limit: 30, offset: 0 }); return ((r && r.pastes) || []).map(x => ({ url: `/p/${x.slug}`, name: x.title || 'Paste', by: x.username || 'anon', meta: x.type === 'screenshot' ? 'image' : (x.language || 'text'), desc: x.ai_summary })); });
...
function _homeMeta() {
    const title = 'HoboStreamer — Free Open-Source Live Streaming Platform';
    const description = 'Free, open-source live streaming with sub-second WebRTC, OBS/RTMP & CLI ingest, auto VODs & AI clips, restreaming to Twitch/YouTube/Kick, global chat, viewer-controlled robots, and 200+ free tools.';
    ...
    try { vods = (db.getRecentVods(12, 0) || []); } catch { /* */ }
    try { clips = (db.getPublicClips(12, 0) || []); } catch { /* */ }
    try { const r = db.listPastes({ visibility: 'public', limit: 12, offset: 0 }); pastes = (r && r.pastes) || []; } catch { /* */ }
    ...
    const vodItems = vods.map(v => ({ url: `/vod/${v.id}`, name: v.title || 'VOD', by: v.display_name || v.username, meta: _fmtDur(v.duration_seconds), desc: v.ai_overview_short }));
    const clipItems = clips.map(c => ({ url: `/clip/${c.id}`, name: c.title || 'Clip', by: c.display_name || c.username || c.streamer_username, meta: _fmtDur(c.duration_seconds) }));
    ...
    const jsonLd = [
        { '@context': 'https://schema.org', '@type': 'ItemList', name: 'Live & recent on HoboStreamer', itemListElement: listEls },
    ];
