/**
 * HoboStreamer — Payments & Subscriptions API  (mounted at /api/payments)
 *
 * - Buy Hobo Bucks via PayPal / Stripe / CCBill / crypto (hosted checkout).
 * - Subscribe to a channel via Stripe (recurring) or Hobo Bucks (30 days).
...
router.post('/subscribe', requireAuth, async (req, res) => {
    ...
    // Pay with Hobo Bucks — instant 30-day sub, no auto-renew.
    if (provider === 'bucks') {
        const cost = pay.bucksForUsd(priceUsd);
        if (!db.deductHoboBucks(req.user.id, cost)) return res.status(402).json({ error: `Not enough Hobo Bucks (need ${cost})` });
        const order = db.createPaymentOrder({
    ...
    return res.status(400).json({ error: 'Subscriptions support Stripe or Hobo Bucks' });
});
