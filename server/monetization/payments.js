/**
 * HoboStreamer — Payments
 *
 * Real-money purchase of Hobo Bucks + channel subscriptions across PayPal,
 * Stripe, CCBill and crypto (NOWPayments). All provider credentials live in
 * site_settings (configured in hobo.tools/admin → Payments). The master switch
...
async function paypalCreateOrder({ order, amountUsd, description }) {
    ...
    const j = await httpJson(`${paypalBase()}/v2/checkout/orders`, {
        ...
        body: JSON.stringify({
            ...
            application_context: {
                brand_name: 'HoboStreamer', user_action: 'PAY_NOW',
