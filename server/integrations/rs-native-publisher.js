/**
 * HoboStreamer — RobotStreamer native publisher (manager)
 *
...
class RsNativePublisher {
    ...
    async _prepareSfuInput(session) {
        ...
        const sdpPath = path.join(os.tmpdir(), `hobo-rs-native-${session.streamId}.sdp`);
    ...
    _buildSdp(videoConsumer, audioConsumer, videoPort, audioPort) {
        const lines = [
            ...
            's=HoboStreamer RS Native Relay',
            'c=IN IP4 127.0.0.1',
            't=0 0',
        ];
