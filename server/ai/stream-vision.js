...
function tmpJpg(streamId) {
    return path.join(os.tmpdir(), `hobo-aisee-${streamId}-${Date.now()}.jpg`);
}
...
async function captureWebrtc(stream) {
    ...
    const sdpLines = [
        'v=0', 'o=- 0 0 IN IP4 127.0.0.1', 's=HoboStreamer AI See',
    ...
    sdpLines.push('');
    const sdpPath = path.join(os.tmpdir(), `hobo-aisee-${stream.id}-${Date.now()}.sdp`);
    const out = tmpJpg(stream.id);
    const cleanup = () => {
