/**
 * HoboStreamer cross-site "went live" notifier.
 * Drop <script src="https://hobostreamer.com/live-notify.js" async></script> on any
 * Hobo site (hobostreamer.com, hobo.tools, hobo.quest, …). It subscribes to the
 * shared SSE feed and shows an on-screen toast with a link to watch — driving
 * viewers to hobostreamer.com. Self-contained: injects its own CSS, no deps.
 */
(function () {
    'use strict';
    if (window.__hoboLiveNotify) return;
    window.__hoboLiveNotify = true;

    var BASE = 'https://hobostreamer.com';
    var seen = {};              // slot key -> last shown ts (client-side dedupe)
    var DEDUPE_MS = 60 * 60 * 1000;
    var AUTO_DISMISS_MS = 13000;

    function injectCSS() {
        if (document.getElementById('hobo-live-notify-css')) return;
        var s = document.createElement('style');
        s.id = 'hobo-live-notify-css';
        s.textContent =
            '#hobo-live-notify{position:fixed;right:16px;bottom:16px;z-index:2147483000;display:flex;flex-direction:column;gap:10px;max-width:340px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}' +
            ...
            '.hln-av{width:42px;height:42px;border-radius:50%;flex:0 0 auto;object-fit:cover;background:#c0965c;display:flex;align-items:center;justify-content:center;color:#111;font-weight:700;font-size:16px;overflow:hidden}' +
    ...
    function ensureContainer() {
        var c = document.getElementById('hobo-live-notify');
        if (!c) {
            c = document.createElement('div');
            c.id = 'hobo-live-notify';
    ...
    function show(d) {
        ...
        card.href = href;
        card.target = (location.host.indexOf('hobostreamer') === -1) ? '_blank' : '_self';
        card.rel = 'noopener';
        card.innerHTML =
            av +
            '<span class="hln-body">' +
