...
function renderContextMenu(menu, profile, username) {
    ...
    const canModerateSiteChat = hasCapability('can_moderate_site_chat');
    ...
    menu.innerHTML = `
        ...
        <div class="ctx-actions">
            <button class="ctx-btn" onclick="ctxWhisper('${esc(username)}')"><i class="fa-solid fa-comment"></i> Whisper</button>
            <button class="ctx-btn" onclick="ctxViewChannel('${esc(username)}')"><i class="fa-solid fa-user"></i> Channel</button>
            ${canModerateSiteChat ? `<button class="ctx-btn" onclick="ctxViewLogs('${esc(username)}', ${profile.id})"><i class="fa-solid fa-clock-rotate-left"></i> Chat Logs</button>` : ''}
            ${canModerateSiteChat ? `<button class="ctx-btn ctx-btn-danger" onclick="ctxBanUser('${esc(username)}', ${profile.id})"><i class="fa-solid fa-ban"></i> Ban</button>` : ''}
        </div>
    `;
}
...
function ctxBanUser(username, userId) {
    ...
    api(`/mod/users/${userId}/ban`, { method: 'POST', body: { reason: 'Banned via chat', duration_hours: 0 } })
        .then(() => toast(`${username} banned`, 'success'))
        .catch((err) => toast(err.message || 'Ban failed', 'error'));
}
...
function openChatLogsModal(username, userId) {
    if (!hasCapability('can_moderate_site_chat')) {
        toast('Staff access required to view site-wide chat logs.', 'error');
        return;
    }
...
async function loadChatLogs() {
    ...
    try {
        ...
        if (ctx.userId && !ctx.query) {
            // User history mode
            url = `/mod/chat/user/${ctx.userId}?limit=50&offset=${ctx.offset}`;
